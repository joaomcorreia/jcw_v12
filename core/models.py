...
class AssistantProfile(models.Model):
    site = models.OneToOneField(Site, on_delete=models.CASCADE, related_name="assistant_profile")
    display_name = models.CharField(max_length=200, default="JCW Assistant")
    role = models.CharField(max_length=240, blank=True)
    purpose = models.TextField(blank=True)
    enabled = models.BooleanField(default=True)
    default_conversation_language = models.CharField(max_length=12, default="nl")
    frontend_enabled = models.BooleanField(default=False)
    backend_enabled = models.BooleanField(default=True)
    business_description = models.TextField(blank=True)
    business_facts = models.JSONField(default=dict, blank=True)
    business_rules = models.JSONField(default=dict, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)

    def __str__(self):
        return f"assistant-profile:{self.site_id}:{self.display_name}"


class AssistantLanguageConfig(models.Model):
    profile = models.ForeignKey(AssistantProfile, on_delete=models.CASCADE, related_name="language_configs")
    language_code = models.CharField(max_length=12)
    enabled = models.BooleanField(default=True)
    ...
    def __str__(self):
        return f"assistant-language:{self.profile_id}:{self.language_code}"


class AssistantCapability(models.Model):
    profile = models.ForeignKey(AssistantProfile, on_delete=models.CASCADE, related_name="capabilities")
    ...
    def __str__(self):
        return f"assistant-capability:{self.profile_id}:{self.key}"
